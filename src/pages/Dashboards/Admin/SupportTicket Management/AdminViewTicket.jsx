...
const AdminViewSupportTicket = () => {
    ...
    const fetchTicket = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setStatus(response.data.supportTicket.status || 'Open');
        }
      } catch (error) {
        Swal.fire({
          icon: 'error',
  ...
  const handleResponseSubmit = async () => {
    setLoading(true)
    try {
      const res = await axios.put(`${import.meta.env.VITE_REACT_APP_API}/api/v1/support-tickets/update/${id}`, { response, status });
      if(res.data.success){
        Swal.fire({
          icon: 'success',
          title: 'Success!',
      ...
      await axios.post(`${import.meta.env.VITE_REACT_APP_API}/api/v1/mail/adminUpdateSupportTicket`, 
        { 
      ...
      <div className="max-w-3xl mx-auto bg-white shadow-xl rounded-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 flex items-center justify-between">
          ...
          <div className='flex flex-col text-white p-4 rounded-lg'>
            <span className='text-2xl font-bold text-white'>User Info: </span>
            <span className='text-blue-100'>Email : {ticket.user.email}</span>
            <span className='text-blue-100'>Username : {ticket.user.username}</span>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-100 p-4 rounded-lg">
            <span className='text-lg font-semibold text-gray-800'>Previous Response</span>
           {ticket.response ? (
             <div className="flex flex-col mt-2">
             <div className="bg-gray-200 p-2 rounded-lg mt-2">
               <p className="text-gray text-xl font-semibold">{ticket.response}</p>
               <span className="text-xm font-semibold text-gray-500">{new Date(ticket.updatedAt).toLocaleString()}</span>
             </div>
           </div>
           ) : (
                <p className="text-gray-600">No previous response</p>
           )}
...
export default AdminViewSupportTicket;
