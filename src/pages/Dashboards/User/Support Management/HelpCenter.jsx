...
const HelpCenter = () => {
  ...
  const categories = [
    ...
    { icon: <MessageCircle className="w-6 h-6" />, title: 'FAQs', description: 'Find answers to common questions', link: '/dashboard/user/faq' },
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((category, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md p-6 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg">
              ...
              <Link  to={category.link || '/dashboard/user/help-center'}
               className="text-purple-600 font-medium flex items-center hover:text-purple-800 transition duration-300 ease-in-out">
        ...
        <div className="mt-12 bg-gradient-to-r from-purple-500 to-blue-600 rounded-lg p-8 text-white">
          ...
          <button className="bg-white text-purple-600 px-6 py-3 rounded-lg font-medium hover:bg-gray-100 transition duration-300 ease-in-out">
            <Link to="/dashboard/user/contact-support">
            Contact Support
            </Link>
          </button>
        </div>
