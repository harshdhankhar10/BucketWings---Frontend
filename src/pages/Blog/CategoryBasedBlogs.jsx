...
const CategoryBasedBlogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API}/api/v1/blogs/all/category/${category}`);
  ...
  return (
    <>
      <Helmet>
        <title>{`${category} Blogs - BucketWings`}</title>
      </Helmet>
      <NavBar />
