...
const PostByCategories = () => {
  ...
  useEffect(() => {
    const fetchPostsByCategory = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/v1/community/posts/category/${category}`);
  ...
  return (
    <>
      <Helmet>
        <title>{category} Posts - Community Forum | BucketWings</title>
      </Helmet>
       <div className="max-w-4xl mx-auto p-6 space-y-6">
      <h1 className="text-3xl font-bold text-center mb-6">Posts in {category}</h1>
      {loading ? (
